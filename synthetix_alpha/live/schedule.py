...
BOOKS = [
    # Both books run the same thing for the last session, sized for expected P&L. n=10 is the best vehicle
    # available: +0.034% per session in the current calm regime, half again SPY's expected return per dollar
    # invested. Expected return then scales with exposure, so 150% is the size and the gate stays off.
