    ...
    <g className={`bar-series-${uniqueId}`}>
      {data.map((d, i) => {
        ...
        if (isHorizontal) {
          ...
          const valuePos = scale(value) ?? 0;
          const zeroY = scale(0) ?? innerHeight;
          barW = barWidth;
          if (!stacked) {
            if (value >= 0) {
              y = valuePos;
              barHeight = Math.max(0, zeroY - valuePos);
            } else {
              y = zeroY;
              barHeight = Math.max(0, valuePos - zeroY);
            }
          } else {
        ...
        const barFill = typeof d.fill === "string" ? d.fill : fill;
        const originY = scale(0) ?? innerHeight;
