...
const ChartCore = memo(function ChartCore({
  ...
  // Compute value extents so negative series (e.g. annual returns) remain visible.
  ...
  }, [innerWidth, innerHeight, valueExtent, isHorizontal]);
  ...
  const yScales = useMemo(() => {
    ...
    return buildYScalesForLines({
      ...
      resolveDomain: (dataKeys) => {
        ...
        const span = Math.max(Math.abs(max), Math.abs(min), 1e-6);
        const pad = span * 0.1;
        return [min < 0 ? min - pad : 0, max > 0 ? max + pad : pad];
      },
    });
